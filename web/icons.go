...
// hubTemplate takes the background colour and the glyph colour.
...
// asset is a pre-generated static file served as-is.
type asset struct {
	data        []byte
	contentType string
}
...
func iconAssets(brandColor string) (map[string]asset, error) {
	...
	for _, v := range variants {
		...
		assets["/icon"+v.suffix+".svg"] = asset{[]byte(svg), "image/svg+xml"}
		assets["/favicon"+v.suffix+".ico"] = asset{ico, "image/x-icon"}
		assets["/apple-touch-icon"+v.suffix+".png"] = asset{touchIcon, "image/png"}
	}

	return assets, nil
}

// icoDirEntry is the 16-byte ICONDIRENTRY header that precedes each image layer.
...
func generateFaviconICO(svg string) ([]byte, error) {
	...
	_ = binary.Write(buf, binary.LittleEndian, []uint16{0, 1, uint16(len(layers))}) //nolint:gosec // two layers

	offset := 6 + 16*len(layers)
	for i, data := range layers {
		_ = binary.Write(buf, binary.LittleEndian, icoDirEntry{
			Width:    sizes[i],
			Height:   sizes[i],
			Planes:   1,
			BitCount: 32,
...
// generateIconPNG rasterises the SVG to a square PNG of the given size.
